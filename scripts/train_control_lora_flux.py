...
def log_validation(flux_transformer, args, accelerator, weight_dtype, step, is_final_validation=False):
    ...
    if not is_final_validation:
        ...
        pipeline = FluxControlRemovalPipeline.from_pretrained(
...
def main(args):
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
        ...
        def load_model_hook(models, input_dir):
            ...
            lora_state_dict = FluxControlRemovalPipeline.lora_state_dict(input_dir)
            transformer_lora_state_dict = {
                f'{k.replace("transformer.", "")}': v
                for k, v in lora_state_dict.items()
                if k.startswith("transformer.") and "lora" in k
            ...
            if args.train_norm_layers:
                ...
                transformer_._transformer_norm_layers = FluxControlRemovalPipeline._load_norm_into_transformer(
                    transformer_norm_state_dict,
    ...
    text_encoding_pipeline = FluxControlRemovalPipeline.from_pretrained(
